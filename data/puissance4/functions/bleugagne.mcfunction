...
execute at @e[name="test",tag=puissance4] run tellraw @a[distance=..30] ["",{"text":"[","color":"gray"},{"text":"Puissance 4","color":"gold"},{"text":"] ","color":"gray"},{"selector":"@a[tag=pbleu]","color":"blue"},{"text":" est le gagnant!","color":"yellow"}]
...
title @a[tag=porange] subtitle {"text":"Vous avez perdu!","color":"red"}
execute as @a[tag=porange] at @s run playsound minecraft:entity.villager.no ambient @s ~ ~ ~ 3
...
title @a[tag=pbleu] subtitle {"text":"Vous avez gagné!","color":"green"}
execute at @e[name="Puissance 4",limit=1,type=armor_stand,tag=done] run playsound minecraft:entity.player.levelup ambient @a[tag=!porange] ~ ~ ~ 3

execute at @e[name="test",tag=puissance4] run setblock ~ ~ ~ minecraft:blue_stained_glass
